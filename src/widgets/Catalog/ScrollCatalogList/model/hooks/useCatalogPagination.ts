...
export const useCatalogPagination = () => {
  ...
  const selectedFilters = useCatalogFiltersStore(
    (state) => state.selectedFilters,
  );
  const selectedRegions = useCatalogFiltersStore(
  ...
  const selectedSort =
    SORT_OPTIONS.find((option) => option.id === selectedSortId)?.value ??
    "default";
  ...
  const {
    ...
    queryFn: () =>
      getCatalogCompanies(debouncedFilters, debouncedRegions, debouncedSortKey),
    staleTime: 5 * 60 * 1000,
  });
